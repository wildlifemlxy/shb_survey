...
var UsersController = require('../../Controller/strawHeadedBulbul/usersController');
...
router.post('/', async function(req, res, next) {
    ...
    const databaseName = requestData.databaseName || "StrawHeadedBulbul";
    ...
    if(requestData.purpose === "retrieve")
    {
        ...
        try {
            ...
            var surveyResult = await surveyController.getAllSurveys(databaseName);
    ...
    else if(requestData.purpose === "getPublicStatistics")
    {
        try {
            ...
            // Volunteers are counted as unique surveyor names within the survey data itself,
            // not Accounts - SHB uses 'Observer name', Rifle Range Road uses 'Name of Surveyors',
            // both stored as comma-separated strings of one or more names.
            const uniqueVolunteers = new Set();
            const addSurveyorNames = (survey) => {
                const raw = survey['Name of Surveyors'] || survey['Observer name'];
                if (!raw) return;
                String(raw).split(',').forEach(name => {
                    const trimmed = name.trim();
                    if (trimmed) uniqueVolunteers.add(trimmed.toLowerCase());
                });
            };

            // Parses either SHB's 'Date' (yyyy-mm-dd) or Rifle Range Road's 'Survey Date' (dd/mm/yyyy)
            const parseSurveyDate = (survey) => {
                if (survey.Date) {
                    const parsed = new Date(survey.Date);
                    if (!isNaN(parsed)) return parsed;
                }
                if (survey['Survey Date']) {
                    const parts = String(survey['Survey Date']).split('/');
                    if (parts.length === 3) {
            ...
            if (surveyResult.surveys && surveyResult.surveys.length > 0) {
                ...
                surveyResult.surveys.forEach(survey => {
                    ...
                    addSurveyorNames(survey);
                    
                    // Find the earliest date
                    const surveyDate = parseSurveyDate(survey);
                    if (surveyDate) {
            ...
            return res.json({
                ...
                userCount: uniqueVolunteers.size
            });
