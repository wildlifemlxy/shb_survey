...
class AddEventModal extends Component {
  ...
  async saveEventsToDatabase(events) {
    ...
    try {
      // Make direct API call to add events
      ...
      if (response.data.success) {
        console.log('Events saved successfully:', response.data.events);
        // Show success message to user
        window.alert(`Successfully added event(s)!`);
        this.handleCancel();
        return response.data.events;
      } else {
        throw new Error(response.data.error || 'Failed to save events');
      }
    } catch (err) {
      ...
      window.alert('Error saving events: ' + errorMessage);
      return null;
    }
  }
