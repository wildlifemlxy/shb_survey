...
import { SATELLITE_PROVIDERS, DEFAULT_SATELLITE_PROVIDER } from '../../config/mapConfig';
...
class SimpleMap extends Component {
  state = {
    ...
    mapStyle: 'terrain' // 'terrain', 'satellite', 'hybrid'
  };
  ...
  renderMapControls = () => {
    const { showHeatMap, showTerrain, showSatellite, selectedActivity, mapStyle } = this.state;
    ...
    return (
      <div style={{
        ...
        <div style={{ marginBottom: '8px' }}>
          <div style={{ fontSize: '11px', fontWeight: 600, marginBottom: '4px', color: '#DC2626' }}>Map Style:</div>
          <select
            value={mapStyle}
          ...
          >
            ...
            <option value="satellite">🛰️ Latest Satellite</option>
            <option value="hybrid">🗺️ Hybrid (Sat + Labels)</option>
          </select>
        </div>
        ...
        <div style={{ marginBottom: '8px' }}>
          ...
          <button
            ...
            title="Refresh for latest satellite imagery"
          >
            🔄 Refresh
          </button>
        </div>
  ...
  render() {
    ...
    const { showHeatMap, showTerrain, selectedActivity, mapStyle } = this.state;
    
    // Get background style based on map style
    const getBackgroundStyle = () => {
      switch (mapStyle) {
        case 'satellite':
          return 'linear-gradient(135deg, #2C3E50 0%, #34495E 20%, #27AE60 40%, #3498DB 60%, #9B59B6 80%, #E74C3C 100%)';
        case 'hybrid':
          return 'linear-gradient(135deg, #34495E 0%, #2ECC71 20%, #3498DB 40%, #9B59B6 60%, #E67E22 80%, #E74C3C 100%)';
        case 'terrain':
        default:
          return showTerrain 
